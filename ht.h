...
public:
    ...
    HASH_INDEX_T next() 
    {
			...
			HASH_INDEX_T temp = ((this->start_) + ((this->numProbes_)*(dhstep_)))%this->m_;
			this->numProbes_++;
			return temp;
...
{
  double fax =  static_cast<double>(delet)/table_.size();
  if (fax >= resizedAlpha_)
  {
    resize();
  }
  ...
  if(npos == index) 
  {
     throw std::logic_error("No location is available");
  }
    if(table_[index] != nullptr){
      if(table_[index]->item.first == p.first){
...
void HashTable<K,V,Prober,Hash,KEqual>::resize()
{
  ...
  for (unsigned int i=0; i < temptable.size(); i++){
    if (temptable[i] != NULL){
      if (temptable[i]->deleted != false){
        delete temptable[i]; 
      }
      else if (temptable[i]!=NULL){
          if(temptable[i]->deleted == false){
            HASH_INDEX_T index = this->probe(temptable[i]->item.first);
            if(npos == index){
                throw std::logic_error("Error");
            }
            table_[index] = temptable[i];
            sizeing++;
            }
        }
    }
